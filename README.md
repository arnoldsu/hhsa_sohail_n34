...
### In Simple Terms: What Does Sohail’s Machine Learning Method (Fully Connected Neural Network (FNN) with Residual Connections) Do?

**Key Point 1: Machine Learning is essentially about trying a large number of numerical combinations very quickly.** Traditional multiple linear regression usually finds one set of coefficients that determines how much weight should be given to each input variable in order to obtain the best possible prediction. The basic idea of a Neural Network is quite similar, except that instead of using only one set of coefficients, it uses many Dense layers to create a large number of different weighted combinations. Each Dense neuron can be thought of simply as an individual calculation with its own set of coefficients. The advantage of a computer is that it can perform a very large number of these calculations simultaneously and very quickly.

**Key Point 2: ReLU (Rectified Linear Unit) can simply be thought of as a switch that determines which of these numerical combinations are passed forward.** After a Dense layer calculates many different combinations, ReLU keeps the positive results and changes the negative results to zero. Therefore, different input data can switch different neurons on or off. In other words, the Neural Network does not always use exactly the same combinations in the same way; different input conditions can activate different combinations. Dense is responsible for mixing and combining the information, while ReLU determines which combinations continue to the next layer. Repeating Dense, ReLU, Dense, ReLU through multiple layers forms what is commonly called a deep neural network. Although the terminology may sound complicated, the basic operation is simply repeating these two processes.

**Key Point 3: Training is like repeatedly adjusting a cookie cutter until it produces the desired shape.** At the beginning, the many weights and biases inside the Neural Network have not yet been properly adjusted, so the first predictions are usually inaccurate. The Network compares its prediction with the correct answer, determines how large the error is, adjusts its internal numbers, and then makes another prediction. This process of prediction, comparison, and adjustment can be repeated very rapidly many times. What the Network eventually “learns” is essentially this large collection of adjusted weights and biases. It is similar to starting with a cookie cutter whose shape is not yet correct and repeatedly making a cookie, comparing it with the desired shape, adjusting the cutter slightly, and trying again. Once Training is completed, new data can be passed through the trained Network to produce predictions very quickly. The important goal is not simply to memorise the training data, but to develop a “cookie cutter” that captures the common patterns well enough to also work with new data that the Network has not previously seen.
